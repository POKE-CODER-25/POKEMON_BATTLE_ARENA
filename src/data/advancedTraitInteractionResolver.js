...
const splitTraitName = (name) =>
  name.split(' + ').map((traitName) => traitName.trim())

const isCopyableTraitName = (traitName) =>
  !UNCOPYABLE_TRAIT_PARTS.some((restrictedName) =>
...
export const getJirachiCopyableTraits = (team = []) =>
  team.flatMap((pokemon) => {
    if (!pokemon?.hasTrait || pokemon.name === 'Jirachi') {
      return []
    }
      ...
      return [
        {
          ...
          traitName: 'Sleeping Monster Copy',
          normalBonus: 10,
          masterRoundBonus: 12,
        },
      ]
    ...
    return (pokemon.traits || []).flatMap((trait) =>
      splitTraitName(trait.name)
        .filter(isCopyableTraitName)
        .map((traitName) => ({
          sourcePokemon: pokemon,
          sourcePokemonName: pokemon.name,
          traitName,
